...
const Product = function Product(cmsconfig) {
  ...
  const handleChange = async (e) => {
    ...
    if (e.target.value !== '') {
      const fetchUrl = `${publicRuntimeConfig.API_BASE_URL}products/search?text.en-us="${e.target.value}"`;
      ...
      if (productData !== undefined) newList = productData.productCTPromise.results;
    } else {
      newList = [];
    }
            ...
            <ul style={dropdownList}>
              {
                ...
                && filterdata.map((product) => (
                  <li className="table-row" key={product.name['en-US']}>
                    <a className="table-cell w-30" onClick={() => selectHandler(product)}>
                      <div className={classes.name} title={product.name['en-US']}>
                        {product.name['en-US']}
                      </div>
                    </a>
                  </li>
